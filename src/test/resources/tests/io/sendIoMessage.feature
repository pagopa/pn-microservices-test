Feature: POST /io/message — Presa in carico sincrona
  ...
  Scenario Outline: Invio messaggio IO e verifica accettazione
    Given un messaggio IO valido con iun "<iun>", recipientTaxId "<recipientTaxId>", senderTaxId "<senderTaxId>", senderServiceId "<senderServiceId>", subject "<subject>", markdown "<markdown>"
    ...
    Examples:
      | iun          | recipientTaxId       | senderTaxId      | senderServiceId       | subject             | markdown                                       |
      | @io.iun      | @io.recipientTaxId   | @io.senderTaxId  | @io.senderServiceId   | Avviso di pagamento | Gentile cittadino, hai ricevuto un avviso. |
  ...
  Scenario Outline: Invio messaggio IO senza iun (campo opzionale) — risposta 202 ACCEPTED
    Given un messaggio IO valido senza iun con recipientTaxId "<recipientTaxId>", senderTaxId "<senderTaxId>", senderServiceId "<senderServiceId>", subject "<subject>", markdown "<markdown>"
    When invio il messaggio IO
    Then la risposta HTTP ha status 202
    And lo status del messaggio è "ACCEPTED"
    And il requestId nella risposta corrisponde a quello inviato
    Examples:
      | recipientTaxId       | senderTaxId      | senderServiceId       | subject             | markdown                                       |
      | @io.recipientTaxId   | @io.senderTaxId  | @io.senderServiceId   | Avviso di pagamento | Gentile cittadino, hai ricevuto un avviso. |
